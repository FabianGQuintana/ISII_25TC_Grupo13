...
public class CD_Pago
{
    ...
    public List<MetodoPago> ListarMetodosPagos()
    {
        ...
        using (var cn = new SqlConnection(Conexion.Cadena))
        {
            string query = "SELECT id_metodo_pago, tipo_pago, descripcion FROM metodo_pago ORDER BY tipo_pago";
            ...
            using (var cmd = new SqlCommand(query, cn))
            {
                ...
                using (var dr = cmd.ExecuteReader())
                {
                    while (dr.Read())
                    {
                        ...
                        {
                            IdMetodoPago = dr["id_metodo_pago"] != DBNull.Value ? (Guid)dr["id_metodo_pago"] : Guid.Empty,
